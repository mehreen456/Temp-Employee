...
//  Temp Provide
//
//  Created by kashif Saeed on 24/05/2017.
...
extension PHPhotoLibrary {
    ...
    fileprivate func saveImage(image: UIImage, album: PHAssetCollection, completion:((PHAsset?)->())? = nil) {
     ...
     func writeImageToFile(image:UIImage) -> String?  {
        
        let documentsDirectoryURL = try! FileManager().url(for: .documentDirectory, in: .userDomainMask, appropriateFor: nil, create: true)
        // create a name for your image
        // writeToFile(, atomically: true)
        let fileURL = documentsDirectoryURL.appendingPathComponent(Constants.S3Credentials.profilePic)
        do{
            try
                UIImageJPEGRepresentation(image, 1.0)!.write(to: fileURL,options:.atomicWrite)
